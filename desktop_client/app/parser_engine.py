...
from .field_mapper import map_dataframe_columns, get_standard_fields
...
def _clean_amount(value: Any) -> float:
    ...
    # 去掉货币符号
    # "$39.37" / "MX$4,299.88" / "￥1,000.00" / "US$39.37"
    s = re.sub(r'^[A-Za-z]{0,3}[\$￥€£¥]?\s*', '', s)

    # 去掉千分位逗号: "1,000.00" → "1000.00"
    # 但要小心欧洲格式 "1.000,00"
    if ',' in s and '.' in s:
        # 判断哪个是千分位，哪个是小数点
        last_dot = s.rfind('.')
...
def _clean_percentage(value: Any) -> float:
    ...
    if value is None or (isinstance(value, float) and np.isnan(value)):
        return 0.0
    if isinstance(value, (int, float)):
        # 如果值 > 1，很可能是百分比形式 (24.65 而非 0.2465)
        if abs(value) > 1:
            return value / 100.0
        return float(value)
