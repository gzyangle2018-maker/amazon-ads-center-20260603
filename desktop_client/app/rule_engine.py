...
from .utils import safe_divide, calc_ctr, calc_cvr, calc_cpc, calc_acos, calc_roas
...
class ActionItem:
    ...
    ad_group_name: str = ""
    target_text: str = ""
    action_layer: str = ""  # 预算调整层/竞价层/否词层/拆精准层...
    current_data: str = ""
    ...
    expected_impact: str = ""
    execute_time: str = ""
    risk_flag: str = ""  # 高相关核心词/需负责人确认/非近3天数据
...
class RuleEngine:
    ...
    def rule_negative_keywords(self, df: pd.DataFrame = None) -> List[Dict]:
        """
        否词规则：
        如果 search_term 或 target_text 点击>=10且订单=0 → 进入否词候选
        """
        if df is None:
        ...
        for _, row in df.iterrows():
            ...
            # 判断否定类型
            # 如果匹配类型是Exact/Broad/Phrase，用对应类型否定
    ...
    def rule_budget(self, df: pd.DataFrame = None, is_recent_3d: bool = False) -> List[Dict]:
        """
        预算规则：
        近3天: ACOS<20% → 预算+100%; ACOS>35% → 预算-50%
        非近3天(7/30天): ACOS<20% → 预算+30~50%; ACOS>35% → 预算-30~50%
        """
        if df is None:
            df = self._get_combined_df([
                'SP_Campaign_Report', 'SB_Campaign_Report'
            ])
    ...
    def rule_bidding_mode(self, df: pd.DataFrame = None) -> List[Dict]:
        ...
        for _, row in df.iterrows():
            ...
            if '仅降低' in bidding or '只降低' in bidding or 'down only' in bidding or '动态竞价' in bidding and '只降低' in bidding:
                campaign = str(row.get('campaign_name', ''))
                results.append({
                    'campaign_name': campaign,
    ...
    # 规则5: 王牌词拆精准规则
    # ============================================================
    def rule_exact_split(self, df: pd.DataFrame = None,
                         target_acos: float = 0.30) -> List[Dict]:
        """规则5：王牌搜索词拆出精准组"""
        if df is None:
            df = self._get_combined_df([
                'SP_Search_Term_Report', 'SB_Search_Term_Report',
    ...
    def run_all_rules(self, is_recent_3d: bool = False) -> AnalysisResult:
        ...
        # 核心诊断
        total_spend = sum(float(a.spend) if hasattr(a, 'spend') else 0 for a in self.actions)
        total_orders = sum(1 for a in self.actions if getattr(a, 'action_layer', '') == '')
        high_priority = sum(1 for a in self.actions if a.priority <= 2)
